...
export default function CombinadoList({
  ...
  const lista = Array.isArray(combinados) ? combinados : [];
  ...
  const filtrados = useMemo(() => {
    ...
    return lista.filter((c) =>
      [c.acolhidoNome, c.acolhidoCpf, rotuloTipoCombinado(c.tipo)].some((valor) =>
  ...
  if (carregando) {
    return <div className="card vazio">Carregando combinados...</div>;
  }

  if (!lista.length) {
    return (
      <div className="card vazio">
        <p>Nenhum combinado cadastrado ainda.</p>
  ...
  return (
    <div className="card tabela-wrapper">
      <div className="lista-cabecalho">
        <h2>Combinados cadastrados ({filtrados.length})</h2>
        <div className="busca-lista">
          <input
            ...
            placeholder="Buscar por acolhido, CPF ou tipo..."
            aria-label="Buscar combinados"
          />
        </div>
        <button
          type="button"
      ...
      {filtrados.length === 0 ? (
        <div className="vazio">Nenhum combinado encontrado para &quot;{busca}&quot;.</div>
      ) : (
              ...
              <tr key={c.id} className={selecionados.has(c.id) ? 'linha-selecionada' : ''}>
                <td className="col-check">
                  <input
                    ...
                    aria-label={`Selecionar combinado de ${c.acolhidoNome ?? ''}`}
                  />
                </td>
                <td>{c.acolhidoNome ?? '-'}</td>
                <td>{c.acolhidoCpf ?? '-'}</td>
                <td>{rotuloTipoCombinado(c.tipo)}</td>
