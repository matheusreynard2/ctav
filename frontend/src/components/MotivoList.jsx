...
// Lista generica de motivos (adesao ou desistencia).
export default function MotivoList({
  motivos,
  carregando,
  titulo,
  rotuloSingular,
  ...
  if (carregando) {
    return <div className="card vazio">Carregando...</div>;
  }

  if (!lista.length) {
    return (
      <div className="card vazio">
        <p>Nenhum {rotuloSingular} cadastrado ainda.</p>
    ...
    <div className="card tabela-wrapper">
      <div className="lista-cabecalho">
        ...
        <button
          ...
          onClick={onNovo}
          title={`Cadastrar ${rotuloSingular}`}
        >
      ...
      {filtrados.length === 0 ? (
        <div className="vazio">Nenhum resultado para &quot;{busca}&quot;.</div>
      ) : (
          ...
          <tbody>
            {itensPagina.map((r) => (
              <tr key={r.id} className={selecionados.has(r.id) ? 'linha-selecionada' : ''}>
                ...
                <td>{r.nome}</td>
                <td title={r.descricao}>{resumir(r.descricao)}</td>
